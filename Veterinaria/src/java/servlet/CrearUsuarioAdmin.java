...
public class CrearUsuarioAdmin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            dto.setPerfilidPerfil(new PerfilDAOImp().PerfilToId(request.getParameter("cmbPerfil")));

            if (!new UsuarioDAOImp().validarRut(dto.getRutUsuario())) {
                
                if (new UsuarioDAOImp().registrarPorAdmin(dto)) {
            ...
            request.getRequestDispatcher("Usuario/crearUsuarioAdmin.jsp").forward(request, response);
