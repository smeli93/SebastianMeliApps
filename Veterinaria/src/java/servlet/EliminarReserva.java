...
public class EliminarReserva extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            dto.setIdReserva(Integer.parseInt(request.getParameter("btnEliminar").trim()));

                if (new ReservaDAOImp().eliminar(dto)) {
                    request.setAttribute("mensaje", "Registro eliminado!!");
                } else {
                    request.setAttribute("mensaje", "No se puede eliminar");
                }
            request.getRequestDispatcher("Reserva/listaReserva.jsp").forward(request, response);
        }
    }
