...
public class EliminarPerfil extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            dto.setIdPerfil(Integer.parseInt(request.getParameter("btnEliminar").trim()));

                if (new PerfilDAOImp().eliminar(dto)) {
                    request.setAttribute("mensaje", "Registro eliminado!!");
                    request.getRequestDispatcher("Perfil/listaPerfil.jsp").forward(request, response);
