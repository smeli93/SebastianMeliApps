...
public class EliminarVeterinario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            dto.setIdVeterinario(Integer.parseInt(request.getParameter("btnEliminar").trim()));

                if (new VeterinarioDAOImp().eliminar(dto)) {
                    request.setAttribute("mensaje", "Registro eliminado!!");
                } else {
                    request.setAttribute("mensaje", "No se puede eliminar");
                }
                
                request.getRequestDispatcher("Veterinario/listaVeterinario.jsp").forward(request, response);
        }
    }
